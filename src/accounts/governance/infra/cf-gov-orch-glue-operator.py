...
def initiate_workflow(event, context):
    ...
    response['GovGlueWorkflowName'] = str(glueWorkflowName)
...
def get_workflow_state(event, context):
    ...
    response = json.dumps({'Status':response['Run']['Status'], 'GovGlueWorkflowName': glueWorkflowName, 'RunId': glueWorkflowRunId})
