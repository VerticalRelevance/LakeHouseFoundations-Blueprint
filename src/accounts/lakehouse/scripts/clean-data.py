...
## @return: DataSource0
## @inputs: []
DataSource0 = glueContext.create_dynamic_frame.from_options(format_options = {"jsonPath":"","multiline":False}, connection_type = "s3", format = "json", connection_options = {"paths": ["s3://dev-lakehouse-lh-s3-raw/"], "recurse":True}, transformation_ctx = "DataSource0")
## @type: ApplyMapping
...
DataSink0 = glueContext.write_dynamic_frame.from_options(frame = DynamicFrame.fromDF(Transform0.toDF().distinct(), glueContext, "unique"), connection_type = "s3", format = "json", connection_options = {"path": "s3://dev-lakehouse-lh-s3-transformed/", "partitionKeys": []}, transformation_ctx = "DataSink0")
job.commit()